...
const AdminEditProductHook = (id) => {
  ...
  const handelSubmit = async (e) => {
    ...
    Array.from(Array(Object.keys(images).length).keys()).map((item, index) => {
      // (دائما اقل من 1000 )لو المستخد ساب صورة للمنتج قديمة و مش هيمسحها
      if (images[index].length <= 1000) {
        // Fetch the image from the URL and convert it to file
        // fetch(images[index])
        //   .then((response) => response.blob())
        //   .then((blob) => {
        //     const file = new File([blob], "image.png", { type: "image/png" });
        //     itemImages.push("image", file, "image.png");
        //   });
        ////////////////////////////////////there is another way to convert old images  to url to send to data base
